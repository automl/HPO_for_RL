...
class HopperConfigModule:
    ENV_NAME = "MBRLHopper-v3"
    TASK_HORIZON = 1000 # Modified from 1000 -> 300
    NTRAIN_ITERS = 300
    NROLLOUTS_PER_ITER = 1
    PLAN_HOR = 15 # Modified from 30 -> 15
    MODEL_IN, MODEL_OUT = 14, 12
    ...
    def obs_cost_fn(obs):
        ...
        # height, ang = obs[1:3]
        # if isinstance(obs, np.ndarray):
        #     alive = (np.isfinite(obs).all() and (np.abs(obs[2:]) < 100).all() and
        #                 (height > .7) and (np.abs(ang) < .2))
        # else:
        #     alive = (tf.reduce_all(tf.math.is_inf(obs)) and (tf.reduce_all(tf.math.abs(obs[2:]) < 100)) and
        #                 (height > .7) and (tf.math.abs(ang) < .2))
        # alive_bonus = 1.0 if alive else 0.0
        
        # return obs_cost - alive_bonus

    @staticmethod
    def ac_cost_fn(acs):
        if isinstance(acs, np.ndarray):
            return 1e-3 * np.sum(np.square(acs), axis=1)
        else:
            return 1e-3 * tf.reduce_sum(tf.square(acs), axis=1)
