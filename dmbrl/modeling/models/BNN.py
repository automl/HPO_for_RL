...
class BNN:
    ...
    ###################################
    def clone(self, state_dict, load_opt_vars=True):
        """
        copy pretrained model by its state_dict to the current model
        Arguments: 
            state_dict: See temp_save function in BNN.py
        """
        # Load min/max logvar from the given model
        if load_opt_vars:
            def get_all_variables_with_name(var_name):
                return [var for var in tf.all_variables() if var.name.endswith(var_name)]

            # Load network parameters
            for key, value in state_dict.items():
                var = get_all_variables_with_name(key)[0]
                var.load(value, self.sess)
        else:
            self.max_logvar.load(state_dict[self.max_logvar.name], self.sess)
            self.min_logvar.load(state_dict[self.min_logvar.name], self.sess)

            # Load scaler
            mu_tf, sigma_tf = self.scaler.get_vars()
            mu_tf.load(state_dict[mu_tf.name], self.sess)
            sigma_tf.load(state_dict[sigma_tf.name], self.sess)

            # Load network parameters
            for i, layer in enumerate(self.layers):
                weights, biases = layer.get_vars()
                weights.load(state_dict[weights.name], self.sess)
                biases.load(state_dict[biases.name], self.sess)

    def temp_save(self, save_opt=True):
        """
        Save all variables in the tensorflow graph into a dictionary
        Arguments:
            save_opt (bool): If True, save the parameters of optimizer, e.g. Adam parameters.
        Return:
            state_dict (dict): dictionary containing all parameters with corresponding names
        """
        state_dict = dict()
        if save_opt:
            for var in tf.all_variables():
                state_dict[var.name] = var.eval(self.sess)
        else:
            # Save log variance
            state_dict[self.max_logvar.name] = self.max_logvar.eval(self.sess)
            state_dict[self.min_logvar.name] = self.min_logvar.eval(self.sess)

            # Save scaler
            mu_tf, sigma_tf = self.scaler.get_vars()
            state_dict[mu_tf.name] = mu_tf.eval(self.sess)
            state_dict[sigma_tf.name] = sigma_tf.eval(self.sess)

            # Load network parameters
            # TODO: Deal with different network structure
            for i, layer in enumerate(self.layers):
                weights, biases = layer.get_vars()
                state_dict[weights.name] = weights.eval(self.sess)
                state_dict[biases.name] = biases.eval(self.sess)
    ...
    def finalize(self, optimizer, optimizer_args=None, *args, **kwargs):
        ...
        Arguments:
            ...
            optimizer_args: (dict) A dic    tionary of arguments for the __init__ method of the chosen optimizer.
        ...
        # self.optimizer_orgs = optimizer_args
        ...
        with tf.variable_scope(self.name):
            ...
            # self.episode_mse_loss = self._compile_episode_loss(self.sy_train_in, self.sy_train_targ)
            self.episode_mse_loss = self._compile_episode_nll_loss(self.sy_train_in, self.sy_train_targ)
    ...
    def train(self, inputs, targets,
        ...
        inputs_shuffled, holdout_inputs_shuffled = inputs[permutation[num_holdout:]], inputs[permutation[:num_holdout]]
        targets_shuffled, holdout_targets_shuffled = targets[permutation[num_holdout:]], targets[permutation[:num_holdout]]
        holdout_inputs = np.tile(holdout_inputs_shuffled[None], [self.num_nets, 1, 1])
        holdout_targets = np.tile(holdout_targets_shuffled[None], [self.num_nets, 1, 1])

        with self.sess.as_default():
            self.scaler.fit(inputs_shuffled)

        idxs = np.random.randint(inputs_shuffled.shape[0], size=[self.num_nets, inputs_shuffled.shape[0]])
        if hide_progress:
        ...
        for _ in epoch_range:
            for batch_num in range(int(np.ceil(idxs.shape[-1] / batch_size))):
                ...
                self.sess.run(
                    self.train_op,
                    # Bootstrapping
                    feed_dict={self.sy_train_in: inputs_shuffled[batch_idxs], self.sy_train_targ: targets_shuffled[batch_idxs]}
                )
            idxs = shuffle_rows(idxs)
            train_loss = self.sess.run(
                    self.mse_loss,
                    feed_dict={
                        self.sy_train_in: inputs_shuffled[idxs[:, :max_logging]],
                        self.sy_train_targ: targets_shuffled[idxs[:, :max_logging]]
                    }
                )
            train_loss_lst.append(train_loss.tolist())
            ...
            if holdout_ratio > 1e-12:
                ...
                test_loss_lst.append(test_loss.tolist())
            else:
                test_loss = None
            ...
            if not hide_progress:
                if holdout_ratio < 1e-12:
                    epoch_range.set_postfix({
                        "Training loss(es)": train_loss
                    })
                else:
                    epoch_range.set_postfix({
                        "Training loss(es)": train_loss,
                        "Holdout loss(es)": test_loss  
                    })
        
        if return_episode_loss:
            idxs = np.tile(np.arange(inputs.shape[0]), [self.num_nets, 1])
            episode_mse_loss = self.sess.run(
                self.episode_mse_loss,
                feed_dict={
                    self.sy_train_in: inputs[idxs[:, :]],
                    self.sy_train_targ: targets[idxs[:, :]]
                }
            )
            episode_loss = episode_mse_loss.tolist()
        else:
            episode_loss = []

        return train_loss_lst, test_loss_lst, episode_loss
    ...
    def _compile_episode_loss(self, inputs, targets):
        """Helper method for compiling the loss function.

        The loss function is obtained from the mean squared error and averaged over each episode
        ...
        Returns: (tf.Tensor) A tensor representing the loss on the input arguments.
        """
        mean, _ = self._compile_outputs(inputs, ret_log_var=True)
        mse_losses = tf.reduce_mean(tf.square(mean - targets), axis=-1)
        return mse_losses
    
    def _compile_episode_nll_loss(self, inputs, targets):
        """Helper method for compiling the loss function.
