...
class MPC(Controller):
    ...
    def train(self, obs_trajs, acs_trajs, rews_trajs):
        ...
        # Train the model
        ret = self.model.train(self.train_in, self.train_targs, **self.model_train_cfg)
        self.has_been_trained = True
        return ret
    
    def eval(self, obs_trajs, acs_trajs, rews_trajs):
        """Trains the internal model of this controller. Once trained,
        this controller switches from applying random actions to using MPC.
        ...
        self.train_targs = np.concatenate([self.train_targs] + new_train_targs, axis=0)
        # Train the model
        ret = self.model.eval(self.train_in, self.train_targs)
        self.has_been_trained = True
        return ret
    ...
    def act(self, obs, t, get_pred_cost=False):
        ...
        # soln has shape [plan_horizon, dU]
    ...
    def reload(self, state_dict, load_opt_vars=True):
        """Copy the model of the given state_dict 
        """        
        # load the model
        self.model.clone(state_dict, load_opt_vars)
        self.has_been_trained = True
    
    def temp_save(self):
        """save the model parameters in a dict
        Return:
            state_dict: (dict) A dict contains all parameters to initialize the model
        """

        state_dict = self.model.temp_save()
        return state_dict

    def save_model(self, path):
        state_dict = self.temp_save()
        np.savez(os.path.join(path, "state_dict.npz"), **state_dict)
    
    def load_model(self, path):
        state_dict = np.load(os.path.join(path, "state_dict.npz"))
        self.reload(dict(state_dict))
